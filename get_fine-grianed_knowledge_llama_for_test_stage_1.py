...
os.environ["OPENAI_BASE_URL"] = "https://api1.uiuiapi.com/v1"
# model = 'gpt-3.5-turbo'
...
client_gpt = OpenAI(
    api_key="sk-QiVT7Ntz8jJEJ5wm1f67392eFf744c4f81FaE20459Ff419f" #"https://uiuiapi.com/v1
...
Entity = namedtuple('Entity', ['name', 'coarse_type', 'fine_type', 'bounding_box'])


def getPrompt(sentence, name, type):
    # print(fine_type)
    coarse_type =  type
    # print(coarse_type)
...
def parse_entity(entity_str):
    ...
    # match = re.match(r'(.+) is a\(an\) (.*?)', entity_str)
    # match_2 = re.match(r'(.+) is a (.*?)', entity_str)
    pattern = r"(?P<name>.+?)\s+is\s+a\(an\)\s+(?P<type>\w+)"
    match = re.search(pattern, entity_str)
    pattern_2 = r"(?P<name>.+?)\s+is\s+a\s+(?P<type>\w+)"

    match_2 = re.search(pattern_2, entity_str)


    if match:
        name = match.group("name")
        coarse_type  = match.group("type")
        ...
        return [name, coarse_type, "NULL","NULL"]
    elif match_2:
        name = match_2.group("name")
        coarse_type = match_2.group("type")
...
def parse_entities(entities_str):
    ...
    for item in entities_list:
        output = parse_entity(item)
        if output==None:
            new_list.append('No entity found.')
        else:
...
#     data = f.readlines()
# with open('./stage1_result/lora_16_checkpoint-700_output.json', 'r', encoding='utf-8') as f:
#     predict_result = f.readlines()
# new_f = open('./test_lora_16_checkpoint-700_output.txt'.format(model), 'w',encoding='utf-8')
# #
# entity_match=0
# predict_number = 0
# golden_number = 0
# predict_entities_all = []
...
#     predict_result_item = eval(predict_result[index])
#
#
#
#     predict_output =predict_result_item["output"]
#     print(predict_output.split(";"))
#
#     predict_entities = parse_entities(predict_output)
#
#     original_item = sentence + "####" + str(predict_entities) + "####" + image_id
#     new_f.write(original_item)
#     new_f.write('\n')
# # exit()
with open('./test_lora_16_checkpoint-700_output.txt', 'r', encoding='utf-8') as f:
    data = f.readlines()


new_f = open('./knowledge/test_{}.json'.format(model), 'a+',encoding='utf-8')

with open('./knowledge/test_{}.json'.format(model), 'r', encoding='utf-8') as f:
